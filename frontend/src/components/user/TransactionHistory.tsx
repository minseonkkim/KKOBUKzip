            ...
            <div>
              ...
              <div className="text-[15px] text-gray-700 flex flex-wrap space-x-1">
                {props.transactionTag?.map((tag, index) => (
                  <span key={index} className="whitespace-nowrap bg-[#D5F0DD] text-[#065F46] px-2 py-1 my-0.5 rounded-full">
                    {tag}
                  </span>
                ))}
              </div>
            ...
            <div className="flex flex-row space-x-3">
              ...
              <div className="text-[18px] font-bold">
                ...
                {(userInfo?.userId !== props.sellerId) && (props.progress === "REVIEW_DOCUMENT") && (
                  <button className="whitespace-nowrap w-auto px-3 h-10 bg-[#E5E4FF] rounded-[10px] hover:bg-[#D6D5F0] hidden" onClick={finalizeTransaction}>
                    구매 확정
                  </button>
                )}
              </div>
