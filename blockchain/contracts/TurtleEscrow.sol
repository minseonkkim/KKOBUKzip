...
contract TurtleEscrow is Ownable {
    ...
    mapping(uint256 => Transaction) public transactions; // 거래 정보 매핑
     ...
     */
    // 새로운 거래 생성
    function createTransaction(uint256 _transactionId, address _seller, uint256 _amount, string memory _turtleId, string memory _buyerId, string memory _sellerId) external returns (uint256) {
        // Check
        ...
        transactions[_transactionId] = Transaction({turtleId: _turtleId, buyerId: _buyerId, sellerId: _sellerId, buyer: msg.sender, seller: _seller, amount: convertedAmount, state: State.Created, createdAt: block.timestamp, lockPeriod: LOCK_PERIOD});
     ...
     * - Interactions: 토큰 전송
     */
    function releaseFunds(uint256 _transactionId) external returns (uint256) {
        // Checks
        Transaction storage transaction = transactions[_transactionId];
     ...
     * - Interactions: 토큰 전송
     */
    function refund(uint256 _transactionId) external returns (uint256) {
        // Checks
        Transaction storage transaction = transactions[_transactionId];
     ...
     * @return 경매 여부, 거래 ID
     */
    function updateLockPeriod(uint256 _transactionId, uint256 _newLockPeriod) external returns (uint256) {
