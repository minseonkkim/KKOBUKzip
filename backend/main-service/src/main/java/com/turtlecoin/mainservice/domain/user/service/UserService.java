        ...
        try {
            ...
            List<UserTurtleResponseDTO> dtos =  user.getTurtles().stream()
                    .map(turtle -> UserTurtleResponseDTO.builder()
                            ...
                            .weight(turtle.getWeight())
                            .gender(turtle.getGender())
                            .motherName(turtle.getMom().getName())
                            .motherImageAddress(turtle.getMom().getTurtlePhotos().get(0).getImage())
                            .fatherName(turtle.getDad().getName())
                            .fatherImageAddress(turtle.getDad().getTurtlePhotos().get(0).getImage())
                            .build())
                    .toList();
            log.info("DTO로 거북이 변환 완료");
            int cnt = dtos.size();
